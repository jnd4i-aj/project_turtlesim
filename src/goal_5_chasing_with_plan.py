...
class turtlebot():
    ...
    def __init__(self):
        ...
        self.real_pose_subscriber = rospy.Subscriber('/rt_real_pose', Pose, self.plan)
    ...
    def plan(self,target):
        ...
        if(self.__goal_distance(target)<=0.5):
            zero_vel=Twist()
            zero_vel.linear.x = 0
            zero_vel.angular.z = 0
            self.velocity_publisher.publish(zero_vel)
            rospy.loginfo("Distance less than 3. Caught RT\n")
            ...
            if(np.shape(a)[0]>=4):
                t1=time.time()
                center,radius=self.__define_circle(a[1],a[2],a[3])	
                plan_length=1
                next_point=self.__next_point(a[2],a[3],center,radius)	
                a.append(next_point)
                plan_length=1
                while(True):
                    ...
                    if(self.__distance(next_point)/self.max_vel>(5*plan_length)):
                        ...
                        rospy.loginfo("Time elapsed in planning: %f",t2-t1)
                        break
